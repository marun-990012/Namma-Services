...
addressController.updateAddress = async(req,res)=>{
    ...
    const url = `https://api.geoapify.com/v1/geocode/search?text=${encodedAddress}&apiKey=${apiKey}`;
    try{
        const address = await Address.findByIdAndUpdate(id,{address:location.formatted,street,city,state,postalCode,country,location: {type: 'Point',coordinates: [location.lon, location.lat]}},{new:true});
        if(!address){
