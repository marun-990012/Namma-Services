...
function EmailVerification() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(
        "Verifictation successful.You can login with your verified email.",
        {
        ...
        <div className="border border-gray-300 shadow rounded-[12px] w-100 px-3 py-6 bg-white">
          <div className="flex justify-center">
            <Mail
              size={70}
              color="green"
              className="mt-[-60px] border-b border-gray-300 shodow p-3 bg-[#e3e7ef] rounded-full"
            />
          </div>
          <div className="text-center">
            <p className="text-[25px]">Please verify your email! </p>
            <p className="mt-3 text-gray-400">
              We sent verifiaction code to{" "}
              <span className="text-black">{regEmail || email}</span>.
            </p>
