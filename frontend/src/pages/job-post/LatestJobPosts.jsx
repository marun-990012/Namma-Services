...
function LatestJobPosts() {
  ...
  const userServiceType = categories?.find((category) => category._id === userAccount?.serviceType);
    ...
    <div className="bg-gray-100 flex justify-center items-center border-3 border-white shadow-xl rounded-[8px] p-10 mb-4">
      <div className="w-full">
        <div className="flex items-center">
          <p>
            Latest Job Posts - <span className="border border-green-300 rounded bg-green-200 text-green-700 font-medium px-3">{filteredNearestJobs.length}</span>
          </p>
          <div className="flex ml-10">
            <div className="flex items-center pl-3 border-2 border-gray-200 rounded bg-orange-400">
              <IconSearch size={18} color="white" className="mr-3" />
              <input type="text" placeholder="Search work" className="focus:outline-none focus:border-none px-3 bg-white w-65 py-[3px]" />
            </div>
          </div>
        </div>

        {Array.isArray(filteredNearestJobs) && filteredNearestJobs.length === 0 ? (
  <div className="flex flex-col items-center justify-center w-full bg-white mt-22 mb-20 p-8 rounded-[10px] shadow-inner border border-gray-200">
    ...
    <h2 className="text-xl font-semibold text-gray-700 mb-1">No Jobs Nearby</h2>
    <p className="text-gray-500 text-sm text-center max-w-md">
      Currently, there are no job posts near your location that you haven't applied to. Please check back later or try exploring other services.
    </p>
  </div>
) : (
  filteredNearestJobs.map((job) => (
    <div key={job._id} className="flex justify-center">
      <div className="flex items-center justify-between bg-white w-full py-2 px-6 rounded-[8px] shadow-md mt-4">
        <div>
          ...
          <div>
            <p className="text-[17px] text-gray-800 font-medium">{job.title}</p>
          </div>
          <div className="mt-1">
          ...
          <div className="flex gap-10 mt-1 text-sm">
            ...
            <p className="text-gray-700 font-semibold">
              Distance: <span className="text-green-600 font-semibold">{job?.dist?.calculated} KM</span>
            </p>
            <p className="text-gray-700 font-semibold">
              Applicants: <span className="text-green-600 font-semibold">{job?.jobRequests?.length}</span>
            </p>
            <p className="text-gray-700 font-semibold">
              Considered: <span className="text-green-600 font-semibold">{job?.considerations?.length}</span>
            </p>
          </div>
        </div>
        <div>
